...
<div class="wrap J_check_wrap">
  ...
  <form name="myform" class="J_ajaxForm" action="{:U("Index/edit")}" method="post" onsubmit="return isSubmit();">
  ...
  <table width="100%" class="table_form contentWrap">
      <tr>
        ...
        <td><select name="typeid" id="typeid" onChange="javascript:field_setting(this.value);">
            ...
            <option value="{$key}" <if condition="$data['typeid'] eq $key"> selected</if>>{$vo}</option>
            </volist>
          </select></td>
      </tr>
      <tr>
        <th><font color="red">*</font> <strong>标题</strong></th>
        <td><input type="text" name="title" id="title" size="60" class="input" value="{$data.title}" /></td>
      </tr>
      <tr>
        <th><font color="red">*</font>  <strong>录入数据</strong></th>
        <td><div id="setting">{$dataForm}</div></td>
      </tr>
    </table>
  </div>
  <div class="btn_wrap">
