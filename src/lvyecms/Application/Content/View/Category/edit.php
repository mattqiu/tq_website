         ...
         <tr>
          ...
          <switch name="vo.type">
             ...
             <case value="file">
                 ...
                 <input type="text" name="extend[{$vo.fieldname}]" value="{$data['setting']['extend'][$vo['fieldname']]}" id="extend_{$vo.fieldname}" style="width:400px;" class="input" placeholder="{$vo.setting.tips}" />  <input type="button" class="button" onclick="flashupload('data_downfile', '附件上传','extend_{$vo.fieldname}',submit_attachment,'1,gif|jpg|jpeg|png|bmp,1','content','-1','{$authkey}')" value="上传文件"> {$vo.setting.tips}
             </case>
          </switch>
...
$(function(){
	...
	$('.add_extend a').click(function(){
      ...
      <?php $authkey = upload_key("1,gif|jpg|jpeg|png|bmp,1"); ?>
          ...
          <th width="120">'+setting.title+'(<a href="javascript:;;" class="extend_del">删除</a>)</th>\
          <th class="y-bg">\
      <input type="text" name="extend['+fieldname+']" id="extend_{$vo.fieldname}" style="width:400px;" class="input" placeholder="'+setting.tips+'" />  <input type="button" class="button" onclick="flashupload(\'data_downfile\', \'附件上传\',\'extend_{$vo.fieldname}\',submit_attachment,\'1,gif|jpg|jpeg|png|bmp,1\',\'content\',\'-1\', \'{$authkey}\')" value="上传文件"> '+setting.tips+'\
		  <input type="hidden" name="extend_config['+fieldname+'][fieldname]" value="'+fieldname+'"/>\
		  <input type="hidden" name="extend_config['+fieldname+'][type]" value="'+type+'"/>\
		  <input type="hidden" name="extend_config['+fieldname+'][setting][title]" value="'+setting.title+'"/>\
		  <input type="hidden" name="extend_config['+fieldname+'][setting][tips]" value="'+setting.tips+'"/>\
